...
export class AdminEditProfileController implements IController {
  constructor(
    private readonly presenter: IResponseHandler,
  ) {}

  public async handle(
    req: RequestModel<AdminEditProfileDTO>,
  ): Promise<Either<BaseError, IResponseModel<any>>> {
		const useCase = container.resolve(AdminEditProfileHandler)
    const userId = req.userId
    if (!userId) throw new Error('Unauthorized')
    const dto = req.body as AdminEditProfileDTO
